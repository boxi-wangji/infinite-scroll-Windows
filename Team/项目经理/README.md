...
| 阶段 | 内容 | 状态 |
|------|------|------|
| M1 | 项目搭建（Vite + Electron + React 跑通） | ⬜ 待开始 |
| M2 | 单个终端面板渲染（node-pty + xterm.js 联调） | ⬜ 待开始 |
| M3 | 无限滚动画布（多行多列布局） | ⬜ 待开始 |
| M4 | 全部快捷键实现 | ⬜ 待开始 |
| M5 | 会话持久化（布局 + cwd + scrollback 恢复） | ⬜ 待开始 |
| M6 | 打包为 Windows 安装包（NSIS） | ⬜ 待开始 |
